...
argument-hint: "[paper filename in raw/ or path to .tex/.pdf]"
...
**Input:** `$ARGUMENTS` — path to a paper (.tex, .pdf, or .qmd), or a filename in `raw/`.
...
6. **Save report to disk** (e.g., `paper_review_[sanitized_name].md`)
